import React, { createRef, forwardRef, Ref, useEffect, useImperativeHandle, useRef, useState } from 'react';
...
import {
  ...
  EquirectangularVideoAdapterConfig,
} from '@photo-sphere-viewer/equirectangular-video-adapter';
...
const ReactPhotoSphereViewer = forwardRef((options: Props, ref: unknown): React.ReactElement => {
  ...
  useEffect(() => {
    ...
    if (sphereElementRef.current && !spherePlayerInstance.current) {
      ...
      if (options.littlePlanet) {
        ...
        if (
          _currentNavbar !== false &&
          !_currentNavbar.find((item) => typeof item === 'object' && item?.id === 'resetLittlePlanetButton')
        ) {
      ...
      Emitter.on('animate', (options: AnimateOptions) => {
        ...
        .on(
          'setOption',
          (pair: {
            option: keyof UpdatableViewerConfig;
            value: UpdatableViewerConfig[keyof UpdatableViewerConfig];
          }) => {
  ...
  return <div className={options.containerClass || 'view-container'} ref={sphereElementRef} />;
});
